...
const deleteProduct = asyncHandler(async (req, res) => {
  ...
  validateMongoDbId(id);
  try {
    const receiptWithProduct = await Receipt.findOne({
      'receiptDetails.productId': id
    });

    if (receiptWithProduct) {
      throw new Error(
        "Xóa sản phẩm không thành công, vì tồn tại nhập hàng liên quan đến sản phẩm này"
      );
    }
...
const getAllProducts = asyncHandler(async (req, res) => {
  try {
    ...
    const productCount = await Product.find(JSON.parse(queryStr)).countDocuments();
    ...
    // Kiểm tra trang có tồn tại
    const totalPages = Math.ceil(productCount / limit);
    if (skip >= productCount) {
      throw new Error("Trang này không tồn tại!");
    }

    // Thực hiện query với pagination và populate
